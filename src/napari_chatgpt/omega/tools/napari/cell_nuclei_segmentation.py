...
**Task:**
Given a plain text request, you competently write a python function segment(viewer) that calls the cellpose_segmentation() or stardist_segmentation() functions.
...
The only segmentation functions that you can use is cellpose_segmentation() and stardist_segmentation():
...
Both functions returns the segmented image as a labels array.
Use function either cellpose_segmentation() or stardist_segmentation() directly without importing or implementing these functions,
they will be provided to you by the system.

**IMPORTANT INSTRUCTIONS:**
- DO NOT include code for the function 'cellpose_segmentation()' or 'stardist_segmentation()' in your answer.
- INSTEAD, DIRECTLY call the segmentation function 'cellpose_segmentation()' or 'stardist_segmentation()'  provided above after import.
- Assume that the function 'cellpose_segmentation()' and 'stardist_segmentation()' are available and within scope of your code.
...
class CellNucleiSegmentationTool(NapariBaseTool):
    ...
    description = (
        ...
        "Input must be a plain text request that must mention 'cellpose' or 'stardist'. "
        "For example, you can request to: 'segment cell nuclei in selected layer with StarDist', or 'segment cell's cytoplams in selected layer with Cellpose'. "
    ...
    def _run_code(self, query: str, code: str, viewer: Viewer) -> str:
        with asection(f"CellNucleiSegmentationTool: query= {query} "):
            ...
            if 'cellpose' in code_lower:
                segmentation_code = _get_seg_code('cellpose')
            elif 'stardist' in code_lower:
