...
package_path = os.path.dirname(skimage_path)   # Replace with the actual package path
...
for root, dirs, files in os.walk(package_path):
    ...
    for filename in files:
        ...
        if filename.endswith('.py'):
            ...
            module_name = os.path.splitext(os.path.relpath(os.path.join(root, filename), package_path))[0].replace('/', '.')
            print(module_name)