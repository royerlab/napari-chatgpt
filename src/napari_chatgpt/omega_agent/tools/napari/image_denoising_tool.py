"""Tool for denoising images using the Aydin library.

This module provides ``ImageDenoisingTool``, an LLM-driven tool that generates
and executes denoising code within a napari viewer.  Two Aydin denoising
approaches are supported: Classic (filter-based) and FGR (Feature Generation
and Regression / Noise2Self).
"""
...
class ImageDenoisingTool(BaseNapariTool):
    """LLM-driven tool for denoising nD images with the Aydin library.

    Supports two denoising approaches:

    * **Classic** -- filter-based methods (Butterworth, bilateral, wavelet, etc.)
    * **FGR** -- Noise2Self Feature Generation and Regression (CatBoost, LGBM, etc.)

    The sub-LLM generates a ``denoise(viewer) -> ArrayLike`` function that
    delegates to one of the available Aydin functions.  The denoised image is
    added to the viewer as a new layer named ``"denoised"``.

    Attributes:
        prompt: Prompt template describing the available denoising functions.
        instructions: Specific instructions for the sub-LLM.
    """

    def __init__(self, **kwargs):
        """Initialize the denoising tool.
    ...
    def _run_code(self, request: str, code: str, viewer: Viewer) -> str:
        """Execute LLM-generated denoising code on the napari viewer.

        Determines which Aydin backend the generated code calls, installs
        Aydin if necessary, prepends the corresponding delegated
        implementation, dynamically imports and runs the ``denoise(viewer)``
        function, and adds the denoised image to the viewer.
...
@cache
def install_aydin():
    """Install the Aydin denoising library via pip (cached, runs at most once).

    Raises:
        NotImplementedError: If running on Apple Silicon (M1/M2), where Aydin
            is not supported.
    """
