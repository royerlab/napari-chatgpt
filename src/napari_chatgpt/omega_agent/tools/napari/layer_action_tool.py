"""Tool for creating napari layer context-menu actions and keyboard shortcuts.

This module provides ``NapariLayerActionTool``, an LLM-driven tool that
generates parameterless layer-action functions.  These functions are
registered as right-click context-menu entries (under an "Omega" submenu)
and optionally bound to keyboard shortcuts via napari's app-model system.
"""
...
class NapariLayerActionTool(BaseNapariTool):
    """LLM-driven tool for creating napari layer actions.

    Generates a parameterless function operating on the active layer, parses
    ``ACTION_*`` metadata comments from the generated code, and registers the
    function as a napari app-model Action in the context menu and (optionally)
    as a keyboard shortcut.

    Attributes:
        code_prefix: Common imports prepended to generated code.
        prompt: Prompt template for the sub-LLM.
        instructions: Detailed coding instructions for the sub-LLM.
    """

    def __init__(self, **kwargs):
        """Initialize the layer-action tool.
    ...
    def _run_code(self, query: str, code: str, viewer: Viewer) -> str:
        """Execute LLM-generated layer-action code and register it in napari.

        Parses metadata comments (title, category, layer type, keybinding),
        dynamically imports the generated function, and registers it with
        napari's app-model as both a context-menu entry and an optional
        keyboard shortcut.

        Args:
            query: The original user request text.
        ...
        try:
            with asection("NapariLayerActionTool:"):
                ...
                clean_code = filter_lines(clean_code, _code_lines_to_filter_out)
                ...
                _action_counter += 1
                action_id = (
                    f"napari-chatgpt.action." f"{function_name}_{_action_counter}"
                )
    ...
    def _register_action(
        ...
        Creates an ``Omega`` submenu in the layer-list context menu (once),
        then registers the action in both the context submenu and the
        appropriate ``Layers > <Category>`` menu bar entry.

        Args:
            action_id: Unique identifier for the action, e.g.
                ``"napari-chatgpt.action.invert_image_1"``.
            title: Short menu label shown to the user.
            category: One of ``filter``, ``segment``, ``transform``, etc.
            layer_type: One of ``image``, ``labels``, ``points``, ``shapes``,
                or ``any``.
            keybinding: Optional keyboard shortcut string, e.g.
                ``"Ctrl+Shift+I"``.
            callback: The callable to invoke when the action is triggered.
        """
        ...
        aprint(f"Registered action '{title}' " f"(id={action_id}) in menus: {menu_ids}")
