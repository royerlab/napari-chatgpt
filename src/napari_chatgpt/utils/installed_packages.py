...
_image_processing_analysis_packages = \
    ['OpenCV','Pillow','PyWavelets', 'scipy', 'networkx', 'tifffile',
     'scikit-image','napari','psygnal','numpy','imagecodecs','dask', 'dask-core',
...
def pip_list(version: bool = False):
    try:
        ...
        if version:
            package_list = [f"{pkg.metadata['name']}=={pkg.metadata['version']}" for pkg in metadata.distributions()]
        else:
...
def conda_list(version: bool = False):
    try:
        ...
        if version:
            package_list = [f"{line.split()[0]}=={line.split()[1]}" for line in output.split('\n') if not line.startswith('#')]
        else:
