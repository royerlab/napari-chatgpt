...
from napari_chatgpt.utils.llm.vision import describe_image, is_vision_available


@pytest.mark.skipif(not is_llm_available(), reason="requires LLM to run")
def test_is_vision_available():
    assert is_vision_available()


@pytest.mark.skipif(
    not is_llm_available() or not is_vision_available(),
...
def test_gpt_vision():
    ...
    assert (
        "logo" in description_1
        or "snake" in description_1
        or "serpent" in description_1
    )
